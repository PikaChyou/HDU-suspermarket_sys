    ...
    <>
      <div className="sche-box">
        ...
        <Table
          ...
          rowKey={(r) => r.Pdt_id}
        ></Table>
        <div className="sche-button-list">
          <button
            onClick={() => {
              setOpen(true);
            }}
          ...
          <button
            onClick={() => {
              const Spl_name = prompt("请输入进货公司名：");

              fetch("http://122.9.12.102:8000/sche", {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify(
                  order.map((item) => ({
                    Pdt_id: item.Pdt_id,
                    Rtk_no: parseInt(item.Order_num),
                    Rtk_time: getCurrentDateTime(),
                    Rtk_price: item.Order_price,
                    Spl_name: Spl_name,
                  }))
                ),
              })
